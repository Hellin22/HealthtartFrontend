...
try {
  ...
  const registerResponse = await axios.post('http://localhost:8080/inbody/register', dataToSend, {
    headers: {
      'Authorization': `Bearer ${token}`,
  ...
  if (error.response) {
    ...
    alert(`인바디 등록 중 오류가 발생했습니다: ${error.response.data.message || error.response.statusText}`);
  } else {
    alert('인바디 등록 중 오류가 발생했습니다. 네트워크 연결을 확인해주세요.');
  }
} finally {
